					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateHost()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Hosts = true">Add</base-button>
								              <modal :show.sync="modal2Hosts">
								  ...
								  <base-input label="VerificationStatus" type="text" placeholder="Enter VerificationStatus" v-model="hostToAdd.verificationStatus"></base-input>
												                  </form>
								                </div>
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Host Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="HostId" type="text" placeholder="Enter HostId" v-model="hostDetails.hostId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Verification Status</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="VerificationStatus" type="text" placeholder="Enter VerificationStatus" v-model="hostDetails.verificationStatus"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateHost() {
      ...
      const jsonData = JSON.stringify(this.hostDetails);
      const res = await HostService.update(jsonData);
...
//        this.getAllHosts();
    ...
    async getHostDetails() {
      try {
			let response = await HostService.get(this.hostId);
