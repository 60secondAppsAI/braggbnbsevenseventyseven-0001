					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateSupportTicket()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2SupportTickets = true">Add</base-button>
								              <modal :show.sync="modal2SupportTickets">
								  ...
								  <base-input label="TicketStatus" type="text" placeholder="Enter TicketStatus" v-model="supportTicketToAdd.ticketStatus"></base-input>
												                  </form>
								                </div>
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Support Ticket Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="SupportTicketId" type="text" placeholder="Enter SupportTicketId" v-model="supportTicketDetails.supportTicketId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Issue Description</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="IssueDescription" type="text" placeholder="Enter IssueDescription" v-model="supportTicketDetails.issueDescription"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Ticket Status</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="TicketStatus" type="text" placeholder="Enter TicketStatus" v-model="supportTicketDetails.ticketStatus"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2SupportTickets = false;
    ...
    async updateSupportTicket() {
      ...
      const jsonData = JSON.stringify(this.supportTicketDetails);
      const res = await SupportTicketService.update(jsonData);
...
//        this.getAllSupportTickets();
    ...
    async getSupportTicketDetails() {
      try {
			let response = await SupportTicketService.get(this.supportTicketId);
			this.supportTicketDetails = response.data;
