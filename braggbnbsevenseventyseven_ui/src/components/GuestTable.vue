        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalGuests = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalGuests">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Guest</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="GuestId" type="text" placeholder="Enter GuestId" v-model="guestToAdd.guestId"></base-input>
  <base-input label="TotalBookings" type="text" placeholder="Enter TotalBookings" v-model="guestToAdd.totalBookings"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="guests" :row-key="record => record.GuestId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <GuestPictureView :guests="guests" />
          </div>
...
import GuestPictureView from './GuestPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'guestId',
          visible: true,
          scopedSlots: { customRender: 'guestId' },
          sorter: true
          //sorter: (a, b) => a.guestId - b.guestId,
          //sorter: (a, b) => a.guestId.localeCompare(b.guestId),
        },
        {
          title: 'Total Bookings',
		dataIndex: 'totalBookings',
          visible: true,
          scopedSlots: { customRender: 'totalBookings' },
          sorter: true
          //sorter: (a, b) => a.totalBookings - b.totalBookings,
          //sorter: (a, b) => a.totalBookings.localeCompare(b.totalBookings),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'guestId',           // Column to sort by
  ...
  methods: {
    ...
    async renderGuestsTable() {
      ...
      for (let i = 0; i < this.guests.length; i++) {
        guestsTableData.push({
          id: i,
          guestId: this.guests[i].guestId,
          year: this.guests[i].year,
          date: this.guests[i].date,
          competitionId: this.guests[i].competitionId,
          guestId: this.guests[i].guestId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-guests',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForGuestsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.guestToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.guestToAdd);
      console.log(jsonData);
      
      const res = await GuestService.addGuest(jsonData);
