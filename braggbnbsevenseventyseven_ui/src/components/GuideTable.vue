        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalGuides = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalGuides">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Guide</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="GuideId" type="text" placeholder="Enter GuideId" v-model="guideToAdd.guideId"></base-input>
  <base-input label="Title" type="text" placeholder="Enter Title" v-model="guideToAdd.title"></base-input>
  <base-input label="Content" type="text" placeholder="Enter Content" v-model="guideToAdd.content"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="guides" :row-key="record => record.GuideId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <GuidePictureView :guides="guides" />
          </div>
...
import GuidePictureView from './GuidePictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'guideId',
          visible: true,
          scopedSlots: { customRender: 'guideId' },
          sorter: true
          //sorter: (a, b) => a.guideId - b.guideId,
          //sorter: (a, b) => a.guideId.localeCompare(b.guideId),
        },
        {
          title: 'Title',
		dataIndex: 'title',
      ...
      sortBy: 'guideId',           // Column to sort by
  ...
  methods: {
    ...
    async renderGuidesTable() {
      ...
      for (let i = 0; i < this.guides.length; i++) {
        guidesTableData.push({
          id: i,
          guideId: this.guides[i].guideId,
          year: this.guides[i].year,
          date: this.guides[i].date,
          competitionId: this.guides[i].competitionId,
          guideId: this.guides[i].guideId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-guides',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForGuidesChanged', this.searchQuery);
		//this.renderGuidesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalGuides = false;

      const currentDate = new Date().getTime();
      this.guideToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.guideToAdd);
      console.log(jsonData);
      
      const res = await GuideService.addGuide(jsonData);
